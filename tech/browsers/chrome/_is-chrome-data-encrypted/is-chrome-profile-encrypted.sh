...
if [ "$total_logins" == "0" ]
then
  echo "!!! No passwords to check !!!" >&2
  exit 1
fi

if [ "$total_encrypted_logins" == "0" ]
then
  echo "!!! No encrypted passwords available !!!" >&2
  exit 1
fi
...
then
  echo "!!! Chrome passwords are NOT being correctly encrypted !!!" >&2
  exit 1
fi
...
if [ "$total_cookies" == "0" ]
then
  echo "!!! No cookies to check !!!" >&2
  exit 1
fi

if [ "$total_encrypted_cookies" == "0" ]
then
  echo "!!! No encrypted cookies available !!!" >&2
  exit 1
fi
...
then
  echo "!!! Chrome cookies are NOT being correctly encrypted !!!" >&2
  exit 1
fi
#
#
